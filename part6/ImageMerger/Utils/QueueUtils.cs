...
namespace ImageMergerService
{
    public sealed class QueueUtils
    {
        ...
        public static bool IsWorkQueue()
        {
            bool result = false;
            try
            {
                if (!ApplicationConfigParameters.GetInstance().isWorkQueue)
                {
                    LoggerUtil.logger.Error("Работа с очередями временно приостановлена сервером MSMQueue!");
                }
                else result = true;
            }
            catch (Exception e)
            ...
            try
            {
                if (!MessageQueue.Exists(messageToServerQueue) || !MessageQueue.Exists(messageFromServerQueue))
                    LoggerUtil.logger.Error(String.Format("Не верно указаны параметры подключения к очередям для сервера MSMQueue:\n" +
                        ...
                        "не найдены на сервере MSMQueue!", messageToServerQueue, messageFromServerQueue));
                else result = true;
            }
            catch (Exception e)
            {
